...
import { extractFacts } from "../../llm/extractFacts";
import OpenAI from "openai";

export const processChunkFacts = internalAction({
  args: {
    chunkId: v.id("chunks"),
  ...
  handler: async (ctx, { chunkId, previousChunkId }) => {
    ...
    const chunk = await ctx.runQuery(
      internal.operations.queries.chunks.getById,
      {
        id: chunkId,
      }
    );
    ...
    await ctx.runMutation(internal.operations.mutations.chunks.updateStatus, {
      chunkId,
      status: "extracting",
    });
    ...
    try {
      ...
      for (const fact of factResponse.facts) {
        const factId = await ctx.runMutation(
          internal.operations.mutations.facts.create,
          {
            chunkId,
            fact: {
      ...
      await ctx.runMutation(
        internal.operations.mutations.chunks.updateContext,
        {
          chunkId,
          context: {
